...
Question, how big is the *difference* between the two?
...
PF: Let $x$ *not* be a node (otherwise it is clearly true). Then
define $w$ and $\lambda$ by:
...
Then $\lambda = (f(x) - p(x))/w(x)$. $\phi$ vanishes at $x, x_0, \dots
x_n$, so by Rolle's theorem there are $n+1$ distinct zeros of $\phi'$,
$n$ distinct zeros of $\phi''$, $n-1$ distinct zeros of $\phi'''$
... So $\phi^({n+1})$ has at least one zero, $\xi$ in $(a,b)$.

But $\phi^{(n_1)} = f^{(n+1)} - p^{(n+1)} - \lambda w^{(n+1)} =
f^{(n+1)} - (n+1)! \lambda. At $\xi$ this becomes:
